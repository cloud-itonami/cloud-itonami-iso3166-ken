(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Kenya-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Kenya-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ke-entity?
                                       true` (near-universal for Kenya
                                       ...
                                       Grounded in Kenyan legal entity
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(PIN/統一資格/IFMIS/PPRA登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ke-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ke-entity? true`, INDEPENDENTLY verify
  `:has-ke-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ke-entity?` ground truth (most Kenya public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ke-entity? e))
                 (not (true? (:has-ke-entity? e))))
        [{:rule :ke-entity-missing
          :detail (str subject " はケニア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- pin-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-pin? true`, INDEPENDENTLY check
  `:pin-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-pin? e))
                 (not (true? (:pin-verified? e))))
        [{:rule :pin-unverified
          :detail (str subject " はPIN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ke-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (pin-unverified-violations request st)
