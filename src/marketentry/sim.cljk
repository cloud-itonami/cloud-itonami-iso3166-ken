...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (KEN, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up ke-entity-missing) ==")
    ...
    (println "== filing/submit eng-4 (ke-entity-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up pin-unverified) ==")
    ...
    (println "== filing/submit eng-5 (pin-unverified -> HARD hold) ==")
