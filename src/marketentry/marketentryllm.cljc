(ns marketentry.marketentryllm
  ...
  the KEN public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ke-entity?=" (:has-ke-entity? e)
                        " pin-verified?=" (:pin-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ke-entity? e))
                              (:has-ke-entity? e))
                          (or (not (:requires-pin? e))
                              (:pin-verified? e)))
                   0.9 0.3)}))
