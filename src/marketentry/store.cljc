(ns marketentry.store
  "SSoT for the KEN market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems KE" :portal "IFMIS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ke-entity? true :has-ke-entity? true
             :requires-pin? true :pin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "KEN" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "IFMIS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ke-entity? true :has-ke-entity? true
             :requires-pin? true :pin-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "IFMIS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "KEN" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "IFMIS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ke-entity? true :has-ke-entity? false
             :requires-pin? true :pin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "KEN" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "IFMIS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ke-entity? true :has-ke-entity? true
             :requires-pin? true :pin-verified? false
             :drafted? false :submitted? false
             :jurisdiction "KEN" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "IFMIS/PPRA"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-ke-entity? has-ke-entity?
                               requires-pin? pin-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-ke-entity?)  (assoc :engagement/requires-ke-entity? requires-ke-entity?)
    (some? has-ke-entity?)       (assoc :engagement/has-ke-entity? has-ke-entity?)
    (some? requires-pin?)    (assoc :engagement/requires-pin? requires-pin?)
    (some? pin-verified?)    (assoc :engagement/pin-verified? pin-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-ke-entity? :engagement/has-ke-entity?
   :engagement/requires-pin? :engagement/pin-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-ke-entity? (boolean (:engagement/requires-ke-entity? m))
     :has-ke-entity? (boolean (:engagement/has-ke-entity? m))
     :requires-pin? (boolean (:engagement/requires-pin? m))
     :pin-verified? (boolean (:engagement/pin-verified? m))
